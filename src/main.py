...
def get_rag_response(query):
    ...
    [출력 형식 및 마크다운 규칙]
    1. **강조**: 중요한 키워드는 반드시 **텍스트** 형식을 사용하세요. (별표와 단어 사이 공백 절대 금지)
    2. **표(Table)**: 개념 비교, 연도별 통계, 특징 나열 시 반드시 마크다운 표를 사용하세요.
    - 표 헤더 아래에 `| :--- | :--- |`를 넣어 가독성을 높이세요.
    3. **가시성**: 답변 내에 백슬래시(\)를 사용하여 마크다운 문법을 무효화하지 마세요.
    """
