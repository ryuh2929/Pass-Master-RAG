...
def get_rag_response(query):
    ...
    [원칙]
    1. 자료 기반: 제공된 [참고 자료]의 '출제 횟수/중요도' 메타데이터를 최우선으로 답변하세요.
    - 출처 언급 시 [참고 자료 1] 같은 임의 번호 대신, 데이터에 포함된 실제 **[ID: 번호]**를 명시하세요.
    2. 지식 보충: 자료가 부족할 경우에만 전문 지식을 활용해 원리나 예시를 추가하세요. 단, 출처를 명확히 구분하세요(예: "보충하자면").
    3. 가성비 답변: 중복 설명을 피하고 핵심 위주로 간결하게 구조화하세요. 불확실한 내용은 지어내지 마세요.
    4. 대상: 모든 답변은 한국어로 작성하며, 정보처리기사 수험생 수준에 맞춰 핵심 용어 위주로 설명하세요.
    """
